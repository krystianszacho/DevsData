...
class String_ extends Scalar {
    ...
    private static function codePointToUtf8(int $num): string {
        ...
        if ($num <= 0x7FF) {
            return String_ . phpchr(($num >> 6) + 0xC0) . chr(($num & 0x3F) + 0x80);
        }
        if ($num <= 0xFFFF) {
            return String_ . phpchr(($num >> 12) + 0xE0) . chr((($num >> 6) & 0x3F) + 0x80) . chr(($num & 0x3F) + 0x80);
        }
        if ($num <= 0x1FFFFF) {
            return String_ . phpchr(($num >> 18) + 0xF0) . chr((($num >> 12) & 0x3F) + 0x80)
